...
class BilibiliUpCrawler:
    ...
    def __init__(self, up_mid, max_videos=MAX_VIDEOS, model_type=None, api_keys=None,
        ...
        # 进度回调：由 TaskManager 注入，用于上报“已处理/总数/当前标题”真实进度
        # （默认 None，CLI 模式不传，保持单类结构不变）
        self.progress_callback = progress_callback

        # 温度/最大 token：允许由统一设置覆盖 config 默认值（桌面端设置面板用）
        ...
        self.dashscope_api_key = (api_keys or {}).get("dashscope", DASHSCOPE_API_KEY)
    ...
    def _init_model_client(self):
        ...
        model_configs = {
            ...
            "dashscope":{"api_key": self.dashscope_api_key,"base_url": "https://dashscope.aliyuncs.com/compatible-mode/v1"}
        }
    ...
    def extract_core_view(self, text, title=""):
        ...
        try:
            ...
            elif self.model_type == "dashscope":
    ...
    def process_all_videos(self):
        ...
        # 上报初始进度（已获取视频列表，等待逐个处理）
        if self.progress_callback:
            try:
                self.progress_callback("fetching", 0, total, "")
            except Exception as cb_err:
                print(f"进度回调异常（已忽略）：{cb_err}")
        
        for i, video_info in enumerate(self.videos):
            print(f"\n--- 处理第 {i+1}/{total} 个视频 ---")
            self.process_video(video_info)
            # 每处理完一个视频即上报真实进度（含无字幕跳过的视频）
            if self.progress_callback:
                try:
                    self.progress_callback("processing", i + 1, total, video_info.get("title", ""))
                except Exception as cb_err:
                    print(f"进度回调异常（已忽略）：{cb_err}")
